...
import { supabaseGetSiteContent, supabaseUpsertSiteContent } from "@/lib/supabaseStore";
...
export async function getSiteContent() {
  if (isSupabaseConfigured()) {
    try {
      const doc = await supabaseGetSiteContent();
...
export async function updateSiteContent(partial) {
  ...
  if (isSupabaseConfigured()) {
    try {
      const doc = await supabaseUpsertSiteContent(next);
